...
public class Main {
	...
	private UserAccount getUserAccount(UserAccount userAccountId) {
		SessionFactory sessionFactory = new Configuration().configure().buildSessionFactory();
		Session session = sessionFactory.openSession();
		session.beginTransaction();

		return userAccountId = (UserAccount) session.get(UserAccount.class, 1);
	}

	// Delete an object
	@SuppressWarnings("unused")
	private void deleteUserAccount(UserAccount userAccountId) {
		SessionFactory sessionFactory = new Configuration().configure().buildSessionFactory();
		Session session = sessionFactory.openSession();
		session.beginTransaction();

		session.delete(userAccountId);
	}

	// Update an object
	@SuppressWarnings("unused")
	private UserAccount updateUserAccount(UserAccount userAccountId) {
		SessionFactory sessionFactory = new Configuration().configure().buildSessionFactory();
		Session session = sessionFactory.openSession();
		session.beginTransaction();

		userAccountId = (UserAccount) session.get(UserAccount.class, 1);
		userAccountId.setUserName("newusername");
		session.update(userAccountId);
